...
# --- 2. BEGINNER DATA LISTS (TOP 5 SELECTIONS) ---
TOP_CRYPTO = ["BTC-USD", "ETH-USD", "SOL-USD", "XRP-USD", "DOGE-USD", "Other (Type Custom)"]
TOP_STOCKS = ["NVDA", "TSLA", "AAPL", "MSFT", "SPY", "Other (Type Custom)"]
...
def get_market_data(ticker):
    """Fetches 6 months of data with caching."""
    try:
...
# --- SMART SEARCH BAR (BEGINNER FRIENDLY) ---
c_type, c_search = st.columns([1, 2.5])
with c_type:
    asset_type = st.selectbox("Market", ["CRYPTO", "STOCKS"], label_visibility="collapsed")

with c_search:
    # Logic: Show Dropdown first. If they pick "Other", show text input.
    options = TOP_CRYPTO if asset_type == "CRYPTO" else TOP_STOCKS
    selected_option = st.selectbox("Select Asset", options, label_visibility="collapsed")
...
with tab4:
    ...
    # Replaced Text Input with MultiSelect for Beginners
