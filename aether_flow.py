...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v26.0", 
    page_icon=" ", 
    initial_sidebar_state="collapsed" 
)

# NEW DB NAME FOR V26
DB_NAME = 'aether_v26.db'
...
def init_db():
    ...
    # V25: BOT TRADES TABLE (Ghost Portfolio)
...
def get_market_data(ticker):
    try:
        time.sleep(0.05)
        df = yf.Ticker(ticker).history(period="1y", interval="1d") # V24: Increased to 1y for backtest
...
def train_brain_advanced(df, epochs=5, batch_size=32, look_back=60):
    ...
    # Feature Engineering: Add RSI to DF for learning
    rsi_s, macd_s, sig_s = calculate_technicals(df)
    df_clean = df.copy()
    df_clean['RSI'] = rsi_s
    df_clean.dropna(inplace=True) # Clean NaNs from indicators
    
    # 3 Dimensions: Close Price, Volume, RSI
    ...
    for i in range(look_back, len(scaled_data)):
        x_train.append(scaled_data[i-look_back:i, :]) # All features
        y_train.append(scaled_data[i, 0]) # Target is still just Price (Index 0)
        
    x_train, y_train = np.array(x_train), np.array(y_train)
    
    # LSTM Architecture (Multivariate)
    model = Sequential()
...
def run_backtest(df, model, scaler, look_back=60):
    ...
    # Iterate through the last 20% of data (Test Set)
    ...
    # Calculate Metrics
    wins = 0
    total_trades = len(trades) // 2 # Approx round trips
...
st.markdown('<div class="main-title">  AETHER APEX v26.0</div>', unsafe_allow_html=True)
            ...
            if AI_AVAILABLE:
                ...
                if st.button("TRAIN NEURAL NET", type="primary", use_container_width=True):
                    with st.spinner(f"Training Multivariate LSTM on {ticker}..."):
                        ...
                        if model:
                            # Save model to session state for Backtest usage
    ...
    with c_trade_main:
        ...
        if neural_pred:
            ...
            <div class='metric-container' style='border-color: {p_color};'>
                <h3>  NEURAL NETWORK PREDICTION (Next Close)</h3>
        ...
        if len(selected_assets) < 2:
            ...
            try:
                with st.spinner("Running Monte Carlo Simulations..."):
                    ...
                    # V26 FIX: Use raw numpy values for calculation to avoid Index/Length mismatch
                    # V26 OPTIMIZATION: Removed matrix diagonal logic for speed on mobile
                    ...
                    # Convert to Numpy for fast, error-free math
                    ...
                    # Optimized Volatility Calculation (No 5000x5000 matrix)
                    # formula: sqrt( sum( (weights @ cov) * weights, axis=1 ) )
...
with tab7:
    ...
    if 'trained_model' in st.session_state and st.session_state['trained_model'] is not None:
        ...
        if st.session_state.get('backtest_results'):
            ...
            fig_eq.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', showlegend=False)
...
with tab8:
    ...
    st.markdown("""
    <div style='background-color: #1c1f26; padding: 15px; border-left: 5px solid #00d2ff; border-radius: 5px;'>
        <strong>PROTOCOL: GHOST_MODE</strong><br>
        This module simulates a High-Frequency Trading (HFT) loop. It executes 'Paper Trades' in a safe, isolated environment.
        <br><em>Status: DISCONNECTED (Click Start to Run Burst)</em>
    </div>
    """, unsafe_allow_html=True)
    ...
    if 'trained_model' in st.session_state and st.session_state['trained_model'] is not None:
        ...
        with c_bot1:
            ...
            if st.button("INITIATE GHOST SEQUENCE (10 CYCLES)", type="primary"):
                ...
                # SIMULATION LOOP
                for i in range(1, 11):
                    # 1. Simulate Price Tick (Random walk based on volatility)
                    last_price = df['Close'].iloc[-1]
                    volatility = df['Close'].pct_change().std()
                    sim_price = last_price * (1 + np.random.normal(0, volatility))
                    
                    # 2. AI Decision
                    # (In a real bot, we'd feed this new price into the LSTM. 
                    # Here we use the pre-calculated prediction for speed/demo)
