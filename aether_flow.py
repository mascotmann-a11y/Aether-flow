...
with tab1:
    if df is not None:
        ...
        # --- NEW PROFESSIONAL CHART LOGIC ---
        # 1. Add SMA for context
        ...
        # Moving Average Line (Blue)
        fig.add_trace(go.Scatter(
            x=df.index, y=df['SMA_50'], mode='lines', 
            line=dict(color='#2196F3', width=1.5), name='Trend (50d)'
        ))
        
        # Layout Updates: Axes Visible, Grid Lines, Clear Labels
        fig.update_layout(
            ...
            yaxis=dict(
                showgrid=True, gridcolor='#222', 
                side='right', # Price on Right like Pro apps
                tickprefix="$"
            ),
            title=dict(text=f"{ticker} Daily Chart", x=0.5, font=dict(size=14, color='#888')),
            showlegend=False,
            dragmode=False, # Keep locked to prevent mobile scroll glitch
            clickmode='none'
        )
