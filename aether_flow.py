...
GLOSSARY = {
    "RSI": "Relative Strength Index. >70 is Expensive. <30 is Cheap.",
...
def generate_report_html(username, cash, net_worth, holdings, journal):
    ...
    <body>
        ...
        {journal.to_html(index=False)}
    </body>
    </html>
    """
    return html

# AI Robo-Advisor Logic (V21)
def diagnose_portfolio(holdings):
    if not holdings:
        return "F", " Portfolio Empty. Start trading to get analysis."
    
    crypto_count = sum(1 for t in holdings if "-USD" in t or "DOGE" in t)
    total_assets = len(holdings)
    crypto_ratio = crypto_count / total_assets if total_assets > 0 else 0
    
    if crypto_ratio > 0.8:
        return "C-", " High Risk! You are overly exposed to Crypto. Consider adding Stocks or Gold."
    elif crypto_ratio < 0.2 and total_assets < 3:
        return "B", "Safe but boring. Add some growth assets."
    elif total_assets >= 5 and 0.3 <= crypto_ratio <= 0.6:
        return "A+", " Excellent Diversification! You have a healthy mix."
    else:
        return "B+", "Good start. Keep diversifying."
...
# TABS
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["Trade", "Chat", "Scan", "Portfolio", "Rankings", "Account", "Intel Center"])
            ...
            if st.button("BUY", type="primary", use_container_width=True):
                if not st.session_state['logged_in']:
                    ...
                    if st.session_state['cash'] >= cost:
                        ...
                        new_lvl = log_trade_db(user, ticker, "BUY", curr_price, shares, cost)
            ...
            if st.button("SELL", use_container_width=True):
                if not st.session_state['logged_in']:
                    ...
                    if st.session_state['holdings'].get(ticker, 0) >= shares:
                        ...
                        update_cash(user, new_cash)
                        update_portfolio(user, ticker, st.session_state['holdings'][ticker])
                        new_lvl = log_trade_db(user, ticker, "SELL", curr_price, shares, revenue)
                        st.session_state['level'] = new_lvl
                        st.session_state['xp'] += 10
...
# --- TAB 2: CHAT (VOICE MODE) ---
with tab2:
    st.info(f"  **Tip:** {random.choice(TRADING_TIPS)}")
    
    st.markdown("###  Voice Command (Beta)")
    audio_val = st.audio_input("Record a Command")
    if audio_val:
        st.success("Audio captured! (Simulation: 'Analyze Bitcoin')")
        prompt = f"Analyze {st.session_state['selected_ticker']}"
    else:
        prompt = st.chat_input("Ask Oracle or Type Command...")
...
# --- TAB 3: SCAN (V21 HEATMAP) ---
with tab3:
    st.subheader("Deep Radar (Top 20)")
    if st.button("SCAN MARKET (TOP 20)", use_container_width=True):
        st.write("Initializing Deep Scan...")
        # Extended list with sectors for heatmap
        watch_data = [
            {"Ticker": "BTC-USD", "Sector": "Crypto"}, {"Ticker": "ETH-USD", "Sector": "Crypto"}, 
            {"Ticker": "SOL-USD", "Sector": "Crypto"}, {"Ticker": "XRP-USD", "Sector": "Crypto"}, 
            {"Ticker": "DOGE-USD", "Sector": "Crypto"}, {"Ticker": "NVDA", "Sector": "Tech"}, 
            {"Ticker": "TSLA", "Sector": "Auto"}, {"Ticker": "AAPL", "Sector": "Tech"}, 
            {"Ticker": "MSFT", "Sector": "Tech"}, {"Ticker": "AMZN", "Sector": "Retail"}, 
            {"Ticker": "GOOG", "Sector": "Tech"}, {"Ticker": "META", "Sector": "Tech"}, 
            {"Ticker": "AMD", "Sector": "Tech"}, {"Ticker": "PLTR", "Sector": "Tech"}, 
            {"Ticker": "COIN", "Sector": "Fintech"}, {"Ticker": "GC=X", "Sector": "Commodity"},
            {"Ticker": "CL=X", "Sector": "Commodity"}, {"Ticker": "EURUSD=X", "Sector": "Forex"},
            {"Ticker": "JPY=X", "Sector": "Forex"}, {"Ticker": "GBPUSD=X", "Sector": "Forex"}
        ]
        
        heatmap_data = []
        progress = st.progress(0)
        
        for i, item in enumerate(watch_data):
            t = item["Ticker"]
            try:
                d = get_market_data(t)
                if d is not None:
                    curr = d['Close'].iloc[-1]
                    open_p = d['Open'].iloc[-1]
                    pct_change = ((curr - open_p) / open_p) * 100
                    heatmap_data.append({
                        "Ticker": t, "Sector": item["Sector"], 
                        "Price": curr, "Change": pct_change, 
                        "AbsChange": abs(pct_change) # Size of block
                    })
            except: pass
            progress.progress((i + 1) / len(watch_data))
            
        df_heat = pd.DataFrame(heatmap_data)
        
        # Interactive Treemap
        fig_treemap = px.treemap(
            df_heat, path=[px.Constant("Market"), 'Sector', 'Ticker'], values='AbsChange',
            color='Change', color_continuous_scale='RdYlGn', color_continuous_midpoint=0,
            hover_data=['Price', 'Change'], title="Market Heatmap (Size = Volatility, Color = Performance)"
        )
        fig_treemap.update_layout(margin=dict(t=50, l=25, r=25, b=25), height=500)
        st.plotly_chart(fig_treemap, use_container_width=True)
        
        st.dataframe(df_heat, use_container_width=True)

# --- TAB 4: PORTFOLIO (V21 DOCTOR) ---
with tab4:
    ...
    with c_port2:
        report_html = generate_report_html(st.session_state['username'], st.session_state['cash'], net, st.session_state['holdings'], st.session_state['journal'])
        st.download_button(" Download Report", data=report_html, file_name="portfolio_report.html", mime="text/html", use_container_width=True)
    ...
    # V21 PORTFOLIO DOCTOR
    if st.button(" DIAGNOSE HEALTH", use_container_width=True):
        grade, advice = diagnose_portfolio(st.session_state['holdings'])
        color_grade = "#ccff00" if "A" in grade else "#ff9900" if "B" in grade else "#ff4444"
        st.markdown(f"""
        <div style="background-color: #1c1f26; padding: 20px; border-radius: 10px; border: 1px solid {color_grade}; text-align: center;">
            <h2 style="color: {color_grade}; margin:0;">Grade: {grade}</h2>
...
# --- TAB 5: RANKINGS (V21 COPY TRADING) ---
with tab5:
    st.header(" Global Rankings")
    df_cash, df_xp = get_leaderboard_data()
    
    r1, r2 = st.columns(2)
    with r1:
        st.subheader("Richest Traders")
        for index, row in df_cash.iterrows():
            c1, c2 = st.columns([3, 1])
            with c1: st.write(f"**{index+1}. {row['username']}** - ${row['cash']:,.2f}")
            with c2: 
                if st.button(f"COPY", key=f"copy_{index}"):
                    if st.session_state['logged_in']:
                        # Simulate winning trade
                        profit = random.randint(50, 500)
                        new_cash = st.session_state['cash'] + profit
                        update_cash(st.session_state['username'], new_cash)
                        st.session_state['cash'] = new_cash
                        log_trade_db(st.session_state['username'], "COPY-TRADE", "WIN", 0, 0, profit)
                        st.toast(f"Copied {row['username']}! Profit: ${profit}")
                        time.sleep(1)
                        st.rerun()
                    else: st.error("Login to copy.")
