...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v30.0", 
    page_icon=" ", 
    initial_sidebar_state="collapsed" 
)

# NEW DB NAME FOR V30
DB_NAME = 'aether_v30.db'
...
# --- 3. CSS STYLING (V30: MATRIX PROTOCOL - NO GRAY) ---
st.markdown("""
<style>
    /* V30 GLOBAL RESET - FORCE GREEN */
    .stApp {
    ...
    /* V30: NUCLEAR TEXT OVERRIDE (Kills all Gray) */
    p, h1, h2, h3, h4, h5, h6, span, div, label, .stMarkdown, .stMarkdown p {
        color: #00ff00 !important;
    ...
    .ticker-wrap {
        ...
        border-bottom: 2px solid #00ff00; padding: 8px; box-sizing: border-box;
    }
    .ticker-move { display: flex; animation: ticker 25s linear infinite; min-width: 200%; }
    @keyframes ticker { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
    .ticker-item { flex: 0 0 auto; padding: 0 2rem; font-weight: bold; color: #ffffff; font-size: 14px; }
    ...
    /* TABS (High Contrast Matrix) */
    .stTabs [data-baseweb="tab-list"] { 
        gap: 5px; background-color: #000000; padding: 5px;
        border-bottom: 1px solid #00ff00;
    }
    .stTabs [data-baseweb="tab"] {
        ...
        padding: 0 20px; border: 1px solid #00ff00;
        margin-right: 5px;
    }
    .stTabs [aria-selected="true"] { 
        background: #00ff00 !important; 
        color: #000000 !important; /* Invert Text for Active */
        border: 1px solid #00ff00 !important;
        font-weight: 900 !important;
    }
    /* Force Tab Text Color Override */
    .stTabs [aria-selected="true"] p { color: #000000 !important; }
    .stTabs [data-baseweb="tab"] p { color: #00ff00 !important; }

    /* INPUTS & LABELS (No Gray Labels) */
    ...
    /* BUTTONS (Green Matrix Style) */
    div.stButton > button {
        background: #000000 !important; 
        color: #00ff00 !important; 
        border: 2px solid #00ff00 !important;
        width: 100%; border-radius: 0px; height: 60px; 
    ...
    div.stButton > button p { color: #00ff00 !important; }
    div.stButton > button:active p { color: #000000 !important; }
    ...
    /* DATAFRAMES (Green Grid) */
    [data-testid="stDataFrame"] {
        border: 1px solid #00ff00;
    }
    
    .asset-header {font-size: 24px; font-weight: 900; color: #ffffff !important; margin: 0;}
    .price-pulse {font-size: 36px; font-weight: 900; color: #00ff00 !important; margin: 0; letter-spacing: -1px;}
...
st.markdown('<div class="main-title">AETHER APEX v30.0</div>', unsafe_allow_html=True)
...
if df is not None:
    ...
    color_hex = "#00ff00" if change >= 0 else "#ff4444"
    st.markdown(f"""
...
with tab1:
    ...
    with c_trade_main:
        ...
        colors = ['#ff4444' if row['Open'] - row['Close'] > 0 else '#00ff00' for index, row in df.iterrows()]
        ...
        fig.update_layout(template="plotly_dark", height=600, margin=dict(l=10, r=10, t=10, b=10), 
                          paper_bgcolor='#000000', plot_bgcolor='#000000', showlegend=False)
        fig.update_xaxes(showgrid=True, gridcolor='#333')
        fig.update_yaxes(showgrid=True, gridcolor='#333', side='right')
        ...
        if neural_pred:
            ...
            p_color = "#00ff00" if diff > 0 else "#ff4444"
            st.markdown(f"""
        ...
        m1, m2 = st.columns(2)
        sig_color = "#00ff00" if "BUY" in signal else "#ff4444" if "SELL" in signal else "#ffffff"
...
with tab4:
    ...
    st.markdown(f"<div class='metric-container'><h3>NET WORTH</h3><h1 style='color:#00ff00'>${net:,.2f}</h1></div>", unsafe_allow_html=True)
        ...
        if len(selected_assets) < 2:
            ...
            try:
                with st.spinner("Running Monte Carlo Simulations..."):
                    ...
                    fig_corr.update_layout(template="plotly_dark", paper_bgcolor='#000000')
                    ...
                    fig_ef.update_layout(template="plotly_dark", paper_bgcolor='#000000', title="Efficient Frontier")
...
with tab5:
    ...
    with col_trophy1:
        st.markdown(f"<div class='metric-container'><h3>LEVEL</h3><h1 style='color:#00ff00'>{lvl}</h1></div>", unsafe_allow_html=True)
    with col_trophy2:
...
with tab6:
    ...
    if 'intel_data' in st.session_state:
        for item in st.session_state['intel_data']:
            st.markdown(f"""
            <div style="background-color: #000000; padding: 10px; border-radius: 5px; border-left: 5px solid {item['color']}; margin-bottom: 10px; border: 1px solid #00ff00;">
                <h4 style="margin:0; color: #ffffff;">{item['title']}</h4>
                <p style="color: {item['color']}; font-weight: bold; margin:0;">{item['tag']}</p>
                <a href="{item['link']}" style="color: #00ff00; font-size: 12px;">Read Source ({item['date']})</a>
            </div>
            """, unsafe_allow_html=True)
    else:
...
with tab7:
    ...
    if 'trained_model' in st.session_state and st.session_state['trained_model'] is not None:
        ...
        if st.session_state.get('backtest_results'):
            ...
            color_res = "#00ff00" if final_bal >= 10000 else "#ff4444"
        ...
        with c_bot1:
            ...
            if st.button("INITIATE GHOST SEQUENCE (10 CYCLES)", type="primary"):
                ...
                for i in range(1, 11):
                    ...
                    color = "#00ff00" if action == "BUY" else "#ff4444" if action == "SELL" else "gray"
                    
                    status_log.markdown(f"""
                    <div style='font-family: monospace; font-size: 14px;'>
                        <span style='color: #00ff00'>[CYCLE {i}/10]</span> 
                        Price: ${sim_price:,.2f} | 
                        AI Target: ${pred:,.2f} | 
