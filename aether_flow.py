...
# --- V22 UPGRADE: ENHANCED AI ENGINE ---
        ...
        with st.expander("🧠 AI Brain Control", expanded=True):
            if AI_AVAILABLE:
                ...
                if st.button("TRAIN NEURAL NET", type="primary", use_container_width=True):
                    with st.spinner(f"Training LSTM on {ticker}..."):
...
with tab4:
    ...
    if st.button("RUN QUANT ANALYSIS", use_container_width=True):
        if len(selected_assets) < 2:
            st.error("⚠️ Please select at least 2 assets to run the optimization.")
        else:
            try:
...
with tab6:
    ...
    if feedparser is None:
        st.error("⚠️ The 'feedparser' library is missing. Please install it to use this feature.")
