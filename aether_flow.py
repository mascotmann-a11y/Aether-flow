...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v21.1", 
    page_icon=" ", 
    initial_sidebar_state="collapsed" 
)

# NEW DB NAME FOR V21.1
DB_NAME = 'aether_v21_1.db'
...
# Session State Defaults (FIXED: Added Journal Initialization)
...
# FIX: Ensure Journal exists for Guests to prevent KeyError
...
# AI Robo-Advisor Logic
def diagnose_portfolio(holdings):
    if not holdings:
        return "F", "⚠️ Portfolio Empty. Start trading to get analysis."
    ...
    if crypto_ratio > 0.8:
        return "C-", "⚠️ High Risk! You are overly exposed to Crypto. Consider adding Stocks or Gold."
...
st.markdown('<div class="main-title">  AETHER APEX v21.1</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    st.markdown("### 🎙️ Voice Command (Beta)")
...
with tab4:
    ...
    c1, c2 = st.columns(2)
    with c1: st.markdown(f"<div class='metric-container'><h3>CASH BALANCE</h3><h2>${st.session_state['cash']:,.2f}</h2></div>",
