...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v33.0", 
    page_icon=" ", 
    initial_sidebar_state="collapsed" 
)

# NEW DB NAME FOR V33 (Fresh Start to Fix Corruption)
DB_NAME = 'aether_v33.db'
...
# --- 3. CSS STYLING (V32: FAIL-SAFE + NEON) ---
st.markdown("""
<style>
    /* V32 GLOBAL RESET */
    .stApp {
    ...
    /* BUTTONS (Black BG + Neon Green Border) */
    div.stButton > button {
        background: #000000 !important; 
        color: #ffffff !important; 
    ...
    /* INPUTS (Black BG + Green Border + White Labels) */
    ...
    /* EXPANDER HEADERS (Dark with Blue Text) */
...
def generate_dummy_data(ticker):
    # V32: Fail-Safe Data Generator
    dates = pd.date_range(end=datetime.now(), periods=365)
    base = 50000 if "BTC" in ticker else 150
    # Random walk
    steps = np.random.normal(0, 0.02, size=len(dates))
    prices = base * (1 + steps).cumprod()
    
    df = pd.DataFrame(index=dates)
    df['Close'] = prices
    df['Open'] = prices * (1 + np.random.normal(0, 0.005, size=len(dates)))
    df['High'] = df[['Close', 'Open']].max(axis=1) * (1 + abs(np.random.normal(0, 0.005, size=len(dates))))
    df['Low'] = df[['Close', 'Open']].min(axis=1) * (1 - abs(np.random.normal(0, 0.005, size=len(dates))))
...
def get_market_data(ticker):
    ...
    try:
        ...
        # V32 FAIL-SAFE: Generate Data if API fails
        df = generate_dummy_data(ticker)
        is_simulated = True
...
st.markdown('<div class="main-title">AETHER APEX v33.0</div>', unsafe_allow_html=True)
...
if not st.session_state['logged_in']:
    with st.expander("  Secure Login / Register", expanded=True):
        ...
        with tab_login:
            l_user = st.text_input("Username").strip() # V33 Sanitization
        ...
        with tab_register:
            r_user = st.text_input("New Username").strip() # V33 Sanitization
...
# --- DATA FETCHING (FAIL-SAFE) ---
df, is_simulated = get_market_data(ticker)

if is_simulated:
    st.warning(f"⚠️ NETWORK ERROR: Displaying SIMULATED DATA for {ticker}. Real market feed is currently unreachable.")
...
if df is not None:
    ...
    # This should barely happen now due to simulation fallback
    st.error("Critical System Failure: Unable to generate data.")
    st.stop()
...
with tab2:
    ...
    if prompt:
        ...
        # V32: Use fail-safe data here too
        d_chat, _ = get_market_data(current_ticker) 
    ...
    if st.button("SCAN MARKET (TOP 20)", use_container_width=True):
        with st.spinner("Scanning Top 20 Assets..."):
            ...
            for i, t in enumerate(watch):
                try:
                    # V32: Use fail-safe fetch
                    d, _ = get_market_data(t)
                    if d is not None:
                        s, p = get_signal(d)
