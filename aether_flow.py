...
# --- V23 UPGRADE: MULTIVARIATE AI ENGINE ---
def train_brain_advanced(df, epochs=5, batch_size=32, look_back=60):
    ...
    # This teaches the AI that High Vol + Low RSI might = Reversal
...
def predict_next_day(model, scaler, df, look_back=60):
    ...
    # Re-create the same feature set
    rsi_s, _, _ = calculate_technicals(df)
    df_clean = df.copy()
    df_clean['RSI'] = rsi_s
    # Fill NaN for prediction time if any (though usually tail is fine)
    ...
    # Predict (returns scaled scalar)
    pred_scalar = model.predict(X_test, verbose=0)
    
    # Inverse Transform Logic for Multivariate
    # Scaler expects 3 columns. We have 1 prediction (Price).
    # We create a dummy row [Pred, 0, 0] to reverse map it.
...
# --- TAB 1: TRADE (V22 UPGRADE: AI BRAIN CONTROL) ---
with tab1:
    ...
    with c_trade_main:
        ...
        # DISPLAY NEURAL PREDICTION IF AVAILABLE
        if neural_pred:
