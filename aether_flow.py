...
# --- AETHER AI SAFETY CHECK ---
# Prevents crash if TensorFlow is missing
try:
...
# --- 1. VISUAL CORE & CONFIG ---
st.set_page_config(layout="wide", page_title="AETHER FLOW v1.0", page_icon="💠")
...
<style>
    ...
    div[data-testid="metric-container"] {
        ...
        border-left: 4px solid #00e5ff; padding: 15px; border-radius: 0px;
    }
    .aether-card {
        background-color: #0f0f0f; border: 1px solid #00e5ff; 
        padding: 20px; text-align: center;
        box-shadow: 0 0 15px rgba(0, 229, 255, 0.1); margin-bottom: 10px;
    }
    .news-card {
        background-color: #111; border-left: 3px solid #00e5ff; 
        padding: 10px; margin-bottom: 10px;
    }
    
    /* TEXT & LINKS */
    h1, h2, h3 {font-family: 'Courier New', monospace; color: #fff;}
...
st.sidebar.markdown("`v1.0 | SYSTEM ONLINE`")
...
if mode == "TERMINAL (PRO)":
    ...
    asset_class = st.sidebar.radio("CLASS", ["STOCKS", "CRYPTO", "FOREX"], horizontal=True)
    ...
    if AI_AVAILABLE:
        ...
        if st.sidebar.button("RETRAIN MODEL"): st.session_state['train_trigger'] = True
    else:
        st.sidebar.warning("⚠️ AI Core Offline (TensorFlow Missing)")
...
def get_hype_score(symbol):
    try:
        ...
        keywords = ['surge', 'soar', 'moon', 'breakout', 'bull', 'buy']
        panic = ['crash', 'plunge', 'dump', 'bear', 'sell', 'warning']
        ...
        label = "HIGH ENERGY" if score > 75 else "LOW ENERGY" if score < 25 else "STABLE"
        return score, label
...
if mode == "TERMINAL (PRO)":
    ...
    if 'train_trigger' in st.session_state and AI_AVAILABLE:
        with st.spinner("💠 AETHER IS LEARNING..."):
            train_brain(df, MODEL_FILE)
    ...
    # Tabs
    t1, t2, t3 = st.tabs(["📊 CHART", "🎮 SIMULATOR", "📓 LOGS"])
    
    with t1:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df.index[-100:], y=df['Close'].iloc[-100:], mode='lines', line=dict(color='#00e5ff'), name='Price'))
        if target > 0:
            fig.add_trace(go.Scatter(x=[df.index[-1] + pd.Timedelta(days=1)], y=[target], mode='markers', marker=dict(color='#fff', size=8), name='AI Target'))
    ...
    with t2:
        ...
        k1, k2 = st.columns(2)
        k1.markdown(f"""<div class="aether-card"><h3>LIQUID CASH</h3><h1>${st.session_state['cash']:,.2f}</h1></div>""", unsafe_allow_html=True)
        k2.markdown(f"""<div class="aether-card"><h3>NET WORTH</h3><h1>${net_worth:,.2f}</h1></div>""", unsafe_allow_html=True)
    ...
    if st.button("RUN SIMULATION"):
        with st.spinner("CALCULATING OPTIMAL FLOW..."):
            ...
            fig = px.pie(values=weights, names=tickers, hole=0.5, title="RECOMMENDED ALLOCATION", color_discrete_sequence=px.colors.sequential.Cyan)
            fig.update_layout(template="plotly_dark")
            st.plotly_chart(fig)

elif mode == "ACADEMY (LEARN)":
    st.title("🎓 AETHER ACADEMY")
    st.markdown("Daily Intelligence & Beginner Training")
    ...
    with ac2:
        ...
        feeds = ["https://finance.yahoo.com/news/rssindex", "https://www.investopedia.com/feedbuilder/feed/getfeed/?feedName=rss_headline"]
        for f in feeds:
            try:
                d = feedparser.parse(f)
                for e in d.entries[:2]:
                    st.markdown(f"""<div class="news-card"><a href="{e.link}" target="_blank">{e.title}</a></div>""", unsafe_allow_html=True)
            except: pass
