...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v18.0", 
    page_icon=" ", 
    initial_sidebar_state="collapsed" 
)

# DB_NAME CHANGED TO FORCE FRESH START
DB_NAME = 'aether_secure.db'
...
def login_user(username, password):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT password, cash FROM users WHERE username=?", (username,))
    data = c.fetchone()
    conn.close()
    if data:
...
def create_user(username, password):
    ...
    try:
        c.execute("INSERT INTO users VALUES (?, ?, ?)", (username, hashed_pw, 100000.00))
        conn.commit()
        success = True
    except:
        success = False
...
st.markdown('<div class="main-title">  AETHER APEX v18.0</div>', unsafe_allow_html=True)

# --- V18.0 UPGRADE: SECURE LOGIN PANEL ---
if not st.session_state['logged_in']:
    with st.expander("🔐 Secure Login / Register", expanded=True):
        ...
        with tab_login:
            ...
            if st.button("LOGIN", use_container_width=True):
                user_cash = login_user(l_user, l_pass)
                if user_cash is not None:
                    st.session_state['username'] = l_user
                    st.session_state['cash'] = user_cash
    ...
    # Logged In Header
    st.info(f"👤 Logged in as: **{st.session_state['username']}** | Balance: **${st.session_state['cash']:,.2f}**")
