...
import os  # <--- CRITICAL FIX: Re-added this missing library

# --- CONFIG & SAFETY ---
st.set_page_config(
    layout="wide", 
    page_title="AETHER FLOW v4.1", 
    page_icon="💠", 
...
st.sidebar.markdown("`v4.1 | PURE LAB`")
...
if mode == "TERMINAL (PRO)":
    ...
    with t1:
        # Beautiful White/Blue Area Chart (No Grid Lines)
        fig = go.Figure()
    ...
    if st.button("RUN CALCULATION"):
        with st.spinner("CRUNCHING NUMBERS..."):
            try:
                # FIX FOR CRASH: Handle new YFinance Data Structure
                data = yf.download(tickers, period="1y")
                
                # Check for MultiIndex (The cause of previous crash)
                if isinstance(data.columns, pd.MultiIndex):
                    ...
                    if 'Close' in data.columns.get_level_values(0):
                        ...
                        st.error("Could not find Close prices.")
                        st.stop()
                else:
