...
st.markdown('<div class="main-title">  AETHER APEX v14.1</div>', unsafe_allow_html=True)
    ...
    if st.button("RUN QUANT ANALYSIS", use_container_width=True):
        if len(selected_assets) < 2:
            ...
            try:
                ...
                # Fetch data and ensure it's just the 'Close' prices
                data = yf.download(selected_assets, period="2y")['Close']
                
                # Handling single-level vs multi-level columns if necessary
                ...
                simulations = 200 # Reduced slightly for performance
                days = 30
                last_price = 100 
                ...
                fig_mc = go.Figure()
                # Plot first 50 lines to keep it clean and fast
