...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v17.1", 
    page_icon=" ", 
    # v17.1 FIX: Sidebar is now expanded by default so beginners can see the Login
...
st.markdown('<div class="main-title">  AETHER APEX v17.1</div>', unsafe_allow_html=True)

# LOGIN SYSTEM (Sidebar)
with st.sidebar:
    st.header("Login / Save")
    input_user = st.text_input("Username", value="Guest")
    if st.button("Load Profile"):
        ...
        if cash_db is None:
            ...
            st.success(f"New User {input_user} Created!")
        else:
...
# --- TAB 1: TRADE (UPDATED: Risk Calc & Comparison) ---
with tab1:
    ...
    if compare_ticker:
        ...
        if comp_df is not None:
            # Normalize to start at same price point for visual comparison
...
with tab5:
    ...
    st.write("To switch users, use the Sidebar.")
