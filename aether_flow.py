...
st.markdown('<div class="main-title">  AETHER APEX v14.0</div>', unsafe_allow_html=True)
...
# --- TAB 4: TOOLS (OPTIMIZER OVERHAUL) ---
with tab4:
    st.subheader("Hedge Fund Optimizer")
    st.caption("Advanced Math: Efficient Frontier, Correlation & Monte Carlo Simulation")
    ...
    if st.button("RUN QUANT ANALYSIS", use_container_width=True):
        if len(selected_assets) < 2:
            ...
            try:
                ...
                # 2. EFFICIENT FRONTIER SIMULATION
                ...
                simulations = 1000
                days = 30
                last_price = 100 # Normalize to $100 start
                ...
                fig_mc = go.Figure()
                fig_mc.add_trace(go.Scatter(data=simulation_df, x=list(range(days)), y=simulation_df.iloc[:, :50], mode='lines', line=dict(color='#2196F3', width=1), opacity=0.1, showlegend=False))
                fig_mc.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', title="1,000 Possible Futures (30 Days)", xaxis_title="Days", yaxis_title="Portfolio Value ($100 Start)")
