...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v19.0", 
    page_icon=" ", 
    initial_sidebar_state="collapsed" 
)

# NEW DB NAME FOR V19 (Prevents conflicts)
DB_NAME = 'aether_v19.db'
...
def init_db():
    ...
    # User Table (Added XP and Level columns)
...
def login_user(username, password):
    ...
    if data:
        if check_hashes(password, data[0]):
            return data # Returns (password, cash, xp, level)
    return None
...
def create_user(username, password):
    ...
    try:
        # Default: $100k cash, 0 XP, "Novice" Level
...
def log_trade_db(username, ticker, action, price, shares, total, notes="Manual"):
    ...
    lvl = add_xp(username, 10) # +10 XP per trade
    return lvl
...
def set_alert_db(username, ticker, price):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    # Condition: "ABOVE" if target > current, else "BELOW". Logic handled in check.
    # For simplicity v19 stores just target.
...
def check_alerts_db(username):
    ...
    for ticker, target, rowid in alerts:
        ...
        try:
            ...
            # Simple Logic: If price is within 1% of target
            if abs(curr - target) / target < 0.01:
                triggered.append(f"🔔 ALERT: {ticker} hit ${target:,.2f}!")
                c.execute("DELETE FROM alerts WHERE rowid=?", (rowid,)) # Remove alert once triggered
        except: pass
...
st.markdown('<div class="main-title">  AETHER APEX v19.0</div>', unsafe_allow_html=True)
...
if not st.session_state['logged_in']:
    with st.expander("🔐 Secure Login / Register", expanded=True):
        ...
        with tab_login:
            ...
            if st.button("LOGIN", use_container_width=True):
                data = login_user(l_user, l_pass) # Returns (password, cash, xp, level)
                if data is not None:
    ...
    # Logged In Header with Gamification Stats
    ...
    with c_trade_main:
        ...
        if AI_AVAILABLE:
            if st.button("  DEEP LEARN (SCAN WEB & TRAIN)", use_container_width=True):
                ...
                # Mock call or reuse deep scan logic
                time.sleep(1)
