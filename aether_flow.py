...
st.set_page_config(
    layout="wide", 
    page_title="AETHER APEX v28.0", 
    page_icon=" ", 
    initial_sidebar_state="collapsed" 
)

# NEW DB NAME FOR V28
DB_NAME = 'aether_v28.db'
...
# --- 3. CSS STYLING (V28 HIGH VISIBILITY UPGRADE) ---
st.markdown("""
<style>
    /* V28 GLOBAL MOBILE RESET & BRIGHTENING */
    .stApp {
        background-color: #050505; 
        color: #ffffff; /* V28: Pure White Text */
        font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;
    }
    
    /* Hide Scrollbars for Mobile Cleanliness */
    ...
    /* Responsive Ticker Tape (V27 Scaled) */
    .ticker-wrap {
        width: 100%; overflow: hidden; background: rgba(0, 0, 0, 0.3); 
        border-bottom: 1px solid #00d2ff; padding: 8px; box-sizing: border-box;
    ...
    /* Tabs (Mobile Friendly & High Visibility) */
    ...
    .stTabs [data-baseweb="tab"] {
        ...
        border-radius: 12px; 
        color: #cccccc; /* V28: Bright Silver for unselected tabs */
        font-size: 13px; font-weight: 600; 
        padding: 0 15px; border: 1px solid rgba(255, 255, 255, 0.05);
        flex: 1 0 auto; 
    }
    ...
    div.stButton > button {
        background: linear-gradient(135deg, #00d2ff 0%, #0078ff 100%) !important; 
        color: #ffffff !important; border: none !important;
        width: 100%; border-radius: 14px; height: 60px; 
    ...
    .asset-header {font-size: 22px; font-weight: 900; color: white; margin: 0;}
...
st.markdown('<div class="main-title">AETHER APEX v28.0</div>', unsafe_allow_html=True)
...
with tab5:
    ...
    with col_trophy1:
        st.markdown(f"<div class='metric-container'><h3>LEVEL</h3><h1 style='color:#ccff00'>{lvl}</h1></div>", unsafe_allow_html=True)
    with col_trophy2:
    ...
    st.markdown('<div class="cc-form" style="background: rgba(255,255,255,0.05); backdrop-filter: blur(10px);">', unsafe_allow_html=True)
...
with tab6:
    ...
    if 'intel_data' in st.session_state:
        for item in st.session_state['intel_data']:
            # V28: Updated link color for visibility
            st.markdown(f"""
            <div style="background-color: rgba(255,255,255,0.05); backdrop-filter: blur(10px); padding: 10px; border-radius: 5px; border-left: 5px solid {item['color']}; margin-bottom: 10px;">
                <h4 style="margin:0;">{item['title']}</h4>
                <p style="color: {item['color']}; font-weight: bold; margin:0;">{item['tag']}</p>
                <a href="{item['link']}" style="color: #cccccc; font-size: 12px;">Read Source ({item['date']})</a>
            </div>
            """, unsafe_allow_html=True)
    else:
...
with tab7:
    ...
    if 'trained_model' in st.session_state and st.session_state['trained_model'] is not None:
        ...
        if st.session_state.get('backtest_results'):
            ...
            fig_eq.update_layout(template="plotly_dark", paper_bgcolor='#050505', showlegend=False)
...
with tab8:
    ...
    st.markdown("""
    <div style='background-color: rgba(255,255,255,0.05); backdrop-filter: blur(10px); padding: 15px; border-left: 5px solid #00d2ff; border-radius: 5px;'>
