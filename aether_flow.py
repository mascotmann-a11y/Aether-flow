...
# --- 4. BACKEND LOGIC (UPGRADED v13.0) ---
...
def calculate_technicals(df):
    """Calculates RSI and MACD for the hybrid signal logic."""
    if df is None or len(df) < 26: return 50, 0, 0  # Default neutral values
    
    # RSI Calculation (Manual to avoid dependency errors)
    delta = df['Close'].diff()
...
def deep_scan_web():
    """Scans 10 News Sources with Expanded Vocabulary."""
    status_text = st.empty()
    progress_bar = st.progress(0)
    
    # Expanded Vocabulary for "Omniscience"
...
def train_brain(df, model_file):
    ...
    # v13.0 UPGRADE: 2-Layer LSTM with Dropout
    model = Sequential()
    ...
    model.fit(x, y, batch_size=32, epochs=3, verbose=0) # Increased epochs for better learning
...
def get_signal(df):
    ...
    if rsi < 30: tech_score += 2 # Oversold -> Buy
    elif rsi > 70: tech_score -= 2 # Overbought -> Sell
...
# --- 5. FRONTEND UI (PRESERVED) ---

st.markdown('<div class="main-title">  AETHER APEX v13.0</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if prompt:
        ...
        if "BUY" in prompt.upper(): response = "AI suggests checking RSI < 30 and Positive News Sentiment first."
    ...
    if st.button("CALCULATE OPTIMAL MIX", use_container_width=True):
        try:
            if not selected_assets:
                ...
                time.sleep(1) 
                # Note: yf.download structure changed in recent versions, ensuring compatibility
